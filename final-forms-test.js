...
async function finalFormsTest() {
  try {
    ...
    const { data: i1, error: e1 } = await supabase.from('incidents').insert(incident1).select();
    if (e1) {
      console.log('❌ Test 1 failed:', e1.message);
      allPassed = false;
    } else {
      console.log('✅ Test 1 passed');
      await supabase.from('incidents').delete().eq('incident_id', i1[0].incident_id);
    }
    ...
    const { data: o1, error: oe1 } = await supabase.from('outreach_logs').insert(outreach1).select();
    if (oe1) {
