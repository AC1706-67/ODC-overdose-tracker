...
class MigrationTester {
  ...
  testLocationParsing() {
    ...
    const testCases = [
      ...
      {
        input: 'Downtown Area 12345',
        expected: { type: 'area', name: 'Downtown Area', zip: '12345' },
      },
      {
        input: 'Park & Ride, Denver, CO 80202',
        expected: {
          type: 'intersection',
          city: 'Denver',
          state: 'CO',
    ...
    testCases.forEach((testCase, index) => {
      console.log(
        `  Test ${index + 1}: "${testCase.input}" -> Expected: ${JSON.stringify(testCase.expected)}`,
      );
  ...
  validateMigrationOrder() {
    ...
    this.migrationFiles.forEach((file, index) => {
      ...
      if (fileName.includes(expectedName)) {
        console.log(
          `✅ File ${index + 1} order correct: ${path.basename(file)}`,
        );
      } else {
        console.warn(
          `⚠️  File ${index + 1} order might be wrong: ${path.basename(file)}`,
        );
      }
    });
  }
  ...
  checkDependencies() {
    ...
    this.migrationFiles.forEach((file) => {
      ...
      const expectedDeps = Object.keys(dependencies).find((key) =>
      ...
      if (expectedDeps && dependencies[expectedDeps]) {
        ...
        if (missingDeps.length === 0) {
          ...
          console.warn(
            `⚠️  Missing dependencies in ${path.basename(file)}: ${missingDeps.join(', ')}`,
          );
        }
      }
    });
  ...
  async runAllTests() {
    ...
    try {
      ...
      console.log('\n📋 Summary:');
      console.log(
        `   - ${this.migrationFiles.length} migration files validated`,
      );
