...
async function createOrganizations() {
  try {
    ...
    const healthNetwork = await supabase.from('organizations').insert({
      name: 'Community Health Network',
      description: 'Healthcare network providing community health services and outreach',
    ...
    if (healthNetwork.error) {
      console.log('❌ Error creating Community Health Network:', healthNetwork.error);
    } else {
      console.log('✅ Community Health Network created:', healthNetwork.data[0].id);
    }
    
    // List all organizations
    console.log('\n3. Listing all organizations...');
    const { data: allOrgs, error: listError } = await supabase.from('organizations').select('*');
    ...
    if (testSubmission.error) {
      ...
      if (testSubmission.data && testSubmission.data[0]) {
        await supabase.from('outreach_logs').delete().eq('id', testSubmission.data[0].id);
