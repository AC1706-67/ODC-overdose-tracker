...
async function analyzeSchema() {
  ...
  const expectedTables = {
    ...
    'team_members': {
      columns: ['id', 'organization_id', 'name', 'email', 'role', 'is_active', 'created_at', 'updated_at'],
      description: 'Team member records with org scoping'
    },
    'locations': {
      columns: ['id', 'organization_id', 'name', 'normalized_name', 'kind', 'line1', 'city', 'state', 'postal_code', 'latitude', 'longitude', 'is_active', 'created_at', 'updated_at'],
      description: 'Location records with normalization'
    },
    'outreach_logs': {
      columns: ['id', 'organization_id', 'user_id', 'occurred_at', 'zip_code', 'location_id', 'legacy_location', 'legacy_team_members', 'kit_types', 'num_kits', 'people_reached', 'males_reached', 'females_reached', 'trip_count', 'team_organization', 'notes'],
    ...
    try {
      ...
      if (error) {
        ...
        if (missingColumns.length > 0) {
          analysis.missing.push(`Table ${tableName} missing columns: ${missingColumns.join(', ')}`);
        }
      }
    } catch (err) {
      analysis.tables[tableName] = {
  ...
  for (const [viewName, description] of Object.entries(expectedViews)) {
    ...
    try {
      ...
      if (error) {
        ...
        console.log(`   ✅ EXISTS${data && data.length > 0 ? ' (has data)' : ' (empty)'}`);
      }
    } catch (err) {
      analysis.views[viewName] = {
        exists: false,
  ...
  if (analysis.missing.length === 0) {
    ...
    analysis.recommendations.push('Schema is complete - ready for production use');
  } else {
    ...
    if (outreachTable && outreachTable.exists && outreachTable.columns) {
      ...
      if (!outreachTable.columns['location_id']?.exists) missingOutreachCols.push('location_id uuid REFERENCES public.locations(id) ON DELETE SET NULL');
      if (!outreachTable.columns['legacy_location']?.exists) missingOutreachCols.push('legacy_location text');
      if (!outreachTable.columns['legacy_team_members']?.exists) missingOutreachCols.push('legacy_team_members text[]');
      ...
      if (missingOutreachCols.length > 0) {
        ...
        missingOutreachCols.forEach((col, index) => {
          console.log(`ADD COLUMN IF NOT EXISTS ${col}${index < missingOutreachCols.length - 1 ? ',' : ';'}`);
        });
    ...
    if (missingViews.length > 0) {
      ...
      console.log('-- (See MIGRATION_INSTRUCTIONS.md for complete view definitions)');
  ...
  const existingTables = Object.values(analysis.tables).filter(t => t.exists).length;
  const totalViews = Object.keys(expectedViews).length;
  const existingViews = Object.values(analysis.views).filter(v => v.exists).length;
  
  console.log(`Tables: ${existingTables}/${totalTables} (${Math.round(existingTables/totalTables*100)}%)`);
  console.log(`Views: ${existingViews}/${totalViews} (${Math.round(existingViews/totalViews*100)}%)`);
  console.log(`Issues: ${analysis.missing.length}`);
  
  const overallHealth = analysis.missing.length === 0 ? 'HEALTHY' : 
                       analysis.missing.length <= 3 ? 'NEEDS ATTENTION' : 'CRITICAL';
