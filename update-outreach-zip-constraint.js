...
async function checkOutreachZipData() {
  try {
    ...
    console.log('');
    console.log(
      'ALTER TABLE outreach_logs DROP CONSTRAINT IF EXISTS outreach_logs_zip_code_check;',
    );
    console.log(
      'ALTER TABLE outreach_logs ADD CONSTRAINT outreach_logs_zip_code_check',
    );
    console.log(
      "  CHECK (zip_code ~ '^[0-9]{5}$' OR upper(btrim(zip_code)) IN ('NA','UNKNOWN') OR zip_code IS NULL);",
    );
  } catch (error) {
