...
async function testOrgContext() {
  try {
    ...
    if (constraints && Array.isArray(constraints)) {
      console.log(`Found ${constraints.length} foreign key constraints on outreach_logs:`);
    ...
    if (testNull.error) {
      ...
      if (testNull.data && testNull.data[0]) {
        await supabase.from('outreach_logs').delete().eq('id', testNull.data[0].id);
      }
    }
    ...
    if (testFake.error) {
      ...
      if (testFake.data && testFake.data[0]) {
        await supabase.from('outreach_logs').delete().eq('id', testFake.data[0].id);
      }
    }
