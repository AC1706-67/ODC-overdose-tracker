  ...
  try {
    ...
    if (incidentsError) {
      ...
      incidents.forEach((incident, i) => {
        console.log(
          `  ${i + 1}. ${incident.zip_code} - ${incident.gender} - ${incident.survival} - Narcan: ${incident.narcan_used} - Created: ${incident.created_at}`,
        );
      });
    }
    ...
    if (viewError) {
      ...
      viewData.forEach((row, i) => {
        console.log(
          `  ${i + 1}. Org: ${row.organization_id || 'null'} - Incidents: ${row.incidents_30d} - Narcan: ${row.with_narcan_30d} - Survived: ${row.survived_30d}`,
        );
      });
    }
    ...
    if (nullOrgError) {
      ...
      nullOrgData.forEach((row, i) => {
        console.log(
          `  ${i + 1}. Incidents: ${row.incidents_30d} - Narcan: ${row.with_narcan_30d} - Survived: ${row.survived_30d} - Deceased: ${row.deceased_30d}`,
        );
      });
    }
