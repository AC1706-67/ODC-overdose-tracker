...
async function testIncidentInsert() {
  try {
    console.log('🧪 Testing incident insertion to find missing required fields...\n');
    ...
    if (error1) {
      ...
      // Clean up
      await supabase.from('incidents').delete().eq('incident_id', test1[0].incident_id);
    }
    ...
    if (error2) {
      ...
      // Clean up
      await supabase.from('incidents').delete().eq('incident_id', test2[0].incident_id);
    }
    ...
    if (error3) {
      ...
      // Clean up
      await supabase.from('incidents').delete().eq('incident_id', test3[0].incident_id);
    }
