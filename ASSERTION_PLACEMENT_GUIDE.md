...
```typescript
async function createIncident(data: IncidentData, orgId: string, userId: string) {
  ...
  logger.assert(data.zip_code.length === 5, 'Zip code must be 5 digits', { zip: data.zip_code });
...
export function useDashboardData() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      logger.assert(activeOrg !== null, 'Active organization required for dashboard');
      logger.assert(activeOrg.id !== undefined, 'Organization ID required');
