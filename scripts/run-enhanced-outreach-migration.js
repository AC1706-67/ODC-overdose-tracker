...
class MigrationRunner {
  ...
  async validateEnvironment() {
    ...
    if (!process.env.SUPABASE_SERVICE_ROLE_KEY && !process.env.EXPO_PUBLIC_SUPABASE_ANON_KEY) {
      throw new Error('SUPABASE_SERVICE_ROLE_KEY or EXPO_PUBLIC_SUPABASE_ANON_KEY environment variable is required');
    }

    // Test database connection
    const { data, error } = await supabase.from('outreach_logs').select('count', { count: 'exact', head: true });
    if (error) {
  ...
  async getPreMigrationStats() {
    ...
    if (error) {
      ...
      const queries = [
        { name: 'total_outreach_logs', query: 'SELECT COUNT(*) as count FROM outreach_logs' },
        { name: 'logs_with_location', query: "SELECT COUNT(*) as count FROM outreach_logs WHERE location IS NOT NULL AND trim(location) != ''" },
        { name: 'logs_with_team_members', query: "SELECT COUNT(*) as count FROM outreach_logs WHERE team_members IS NOT NULL AND trim(team_members) != ''" },
        { name: 'existing_locations', query: 'SELECT COUNT(*) as count FROM locations WHERE is_active = true' },
        { name: 'existing_team_members', query: 'SELECT COUNT(*) as count FROM team_members WHERE is_active = true' }
      ];

      const stats = {};
      for (const query of queries) {
        try {
          const { data: result, error: queryError } = await supabase.rpc('exec_sql', { sql: query.query });
  ...
  async checkMigrationStatus() {
    ...
    try {
      ...
      if (error) {
        console.log('ℹ️  Migration log table not found - migration not started');
  ...
  async executeMigrationFile(filePath) {
    ...
    console.log(`✅ Executed ${executedCount} statements from ${path.basename(filePath)}`);
  }
  ...
  async runMigration() {
    ...
    try {
      ...
      console.error('💡 Use the rollback command if needed: node scripts/run-enhanced-outreach-migration.js rollback');
      process.exit(1);
    }
  }
  ...
  async rollbackMigration() {
    ...
    console.log('⚠️  This will remove all migrated data and restore the original state');
    
    // In a real implementation, you might want to add a confirmation prompt
    console.log('🔄 Executing rollback...');
    
    try {
      const { error } = await supabase.rpc('rollback_enhanced_outreach_migration');
      if (error) {
        throw error;
      }
...
async function main() {
  ...
  try {
    switch (command) {
      ...
      default:
        console.log('Usage: node scripts/run-enhanced-outreach-migration.js [validate|migrate|status|rollback]');
        process.exit(1);
    }
  } catch (error) {
