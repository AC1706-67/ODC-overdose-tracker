...
async function permanentFormsFix() {
  try {
    ...
    const { error: alterError } = await supabase.rpc('exec_sql', {
      sql_query: makeClientIdOptional,
    });
    if (alterError && !alterError.message.includes('does not exist')) {
      console.log(
        '⚠️  Could not alter client_id (may not be needed):',
        alterError.message,
      );
    } else {
    ...
    if (incidentError) {
      ...
      if (incidentError2) {
        ...
        await supabase
          .from('incidents')
          .delete()
          .eq('incident_id', incidentResult2[0].incident_id);
      }
    } else {
    ...
    console.log(
      'Both forms should now work consistently without back-and-forth issues.',
    );
  } catch (err) {
