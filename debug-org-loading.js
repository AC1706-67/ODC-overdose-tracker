  ...
  try {
    ...
    if (userOrgsError) {
      ...
      userOrgs.forEach((uo) => {
        console.log(`\nUser: ${uo.user_id}`);
        console.log(
          `Org: ${uo.organizations?.name} (${uo.organizations?.slug})`,
        );
    ...
    orgs.forEach((org) => {
      ...
      const matchesSlug = [
        ...
        'recovery-alliance',
        'raep',
      ].includes(org.slug?.toLowerCase());
      const matchesName = [
        'recovery alliance of el paso',
        'recovery alliance',
      ].includes(org.name?.toLowerCase());
      ...
      console.log(
        `  Should have outreach: ${matchesId || matchesSlug || matchesName ? 'YES' : 'NO'}`,
      );
    });
  } catch (error) {
