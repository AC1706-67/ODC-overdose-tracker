...
async function testOnboardingFlow() {
  ...
  if (validError) {
    ...
    const isValid = validCode.is_active && 
    ...
    if (isValid) {
      ...
      console.log(`   - Usage: ${validCode.current_uses}${validCode.max_uses ? `/${validCode.max_uses}` : ' (unlimited)'}`);
    } else {
      console.log('⚠️  Code exists but is not valid:');
      console.log(`   - Active: ${validCode.is_active}`);
      console.log(`   - Expired: ${validCode.expires_at ? new Date(validCode.expires_at) < new Date() : false}`);
      console.log(`   - Usage limit reached: ${validCode.max_uses && validCode.current_uses >= validCode.max_uses}`);
    }
    console.log();
  }
  ...
  try {
    ...
    const { data: funcTest, error: funcError } = await supabase
      .rpc('increment_invite_code_usage', { code_text: 'NONEXISTENT_CODE' });
    ...
    if (funcError) {
      if (funcError.message.includes('function') && funcError.message.includes('does not exist')) {
        ...
        console.log('✅ Function exists (returned NULL for invalid code as expected)');
        console.log();
      }
    } else {
