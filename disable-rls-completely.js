...
async function disableAllRLS() {
  try {
    ...
    const tests = [
      { table: 'organizations', query: supabase.from('organizations').select('count').limit(1) },
      { table: 'outreach_logs', query: supabase.from('outreach_logs').select('count').limit(1) },
      { table: 'user_organizations', query: supabase.from('user_organizations').select('count').limit(1) }
    ];
    ...
    console.log('⚠️  Note: RLS is disabled for development. Re-enable with proper policies later.');
