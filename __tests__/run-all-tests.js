...
const { EnhancedOutreachAnalyticsTests } = require('./enhanced-outreach-analytics.test.js');
const { DashboardComponentTests } = require('./dashboard-components.test.js');
const { AnalyticsPerformanceTests } = require('./analytics-performance.test.js');
...
class TestRunner {
  ...
  async runTestSuite(suiteName, TestClass) {
    ...
    if (success) {
      this.log(`✅ Test suite ${suiteName} completed successfully in ${duration}ms`, 'success');
    } else {
  ...
  generateOverallReport() {
    ...
    this.log('ENHANCED OUTREACH ANALYTICS - COMPREHENSIVE TEST REPORT', 'summary');
    this.log('='.repeat(80), 'summary');
    
    this.log(`Total execution time: ${Math.round(totalDuration / 1000)}s`, 'info');
    this.log(`Test suites run: ${this.overallResults.suites.length}`, 'info');
    this.log(`Total tests passed: ${this.overallResults.totalPassed}`, 'success');
    this.log(`Total tests failed: ${this.overallResults.totalFailed}`, this.overallResults.totalFailed > 0 ? 'error' : 'success');
    ...
    this.overallResults.suites.forEach(suite => {
      ...
      this.log(`  ${status} ${suite.name}: ${suite.passed}/${suite.passed + suite.failed} tests passed (${duration}s)`, 'info');
    ...
    const successRate = totalTests > 0 ? (this.overallResults.totalPassed / totalTests) * 100 : 0;
    
    this.log('', 'info');
    this.log(`OVERALL SUCCESS RATE: ${successRate.toFixed(1)}%`, successRate >= 80 ? 'success' : 'warning');
    ...
    if (successRate >= 95) {
      this.log('🎉 Excellent! All systems are ready for deployment.', 'success');
    } else if (successRate >= 80) {
      this.log('✅ Good! Review failed tests and warnings before deployment.', 'warning');
      this.log('   Most functionality should work correctly.', 'info');
    } else if (successRate >= 60) {
      this.log('⚠️  Moderate issues detected. Address failed tests before deployment.', 'warning');
      this.log('   Core functionality may be impacted.', 'warning');
    } else {
      this.log('❌ Significant issues detected. Do not deploy until tests pass.', 'error');
      ...
      failedSuites.forEach(suite => {
        switch (suite.name) {
          case 'Enhanced Outreach Analytics Core':
            this.log('  - Core analytics functionality has issues. Check database schema and migrations.', 'error');
            break;
          case 'Dashboard Components Integration':
            this.log('  - Dashboard components have issues. Check component files and API integration.', 'error');
            break;
          case 'Analytics Performance':
            this.log('  - Performance issues detected. Optimize queries and check database indexes.', 'error');
            break;
        }
      });
    }
  ...
  async runAllTests() {
    ...
    const testSuites = [
      { name: 'Enhanced Outreach Analytics Core', class: EnhancedOutreachAnalyticsTests },
      { name: 'Dashboard Components Integration', class: DashboardComponentTests },
