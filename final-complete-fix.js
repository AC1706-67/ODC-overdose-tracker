    ...
    if (allTables && Array.isArray(allTables)) {
      ...
      for (const table of allTables) {
        ...
        try {
          ...
          console.log(
            `  ✅ ${tableName} - RLS disabled, policies removed, permissions granted`,
          );
        } catch (error) {
          console.log(
            `  ⚠️  ${tableName} - Error (may be expected):`,
            error.message,
          );
        }
      }
    ...
    if (testIncident.error) {
      ...
      if (testIncident.data && testIncident.data[0]) {
        await supabase
          .from('incidents')
          .delete()
          .eq('incident_id', testIncident.data[0].incident_id);
      }
    }
