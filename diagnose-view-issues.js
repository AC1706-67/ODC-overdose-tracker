...
async function diagnoseViewIssues() {
  try {
    ...
    const { data: incidentsColumns, error: colError } = await supabase.rpc(
      'sql',
      {
        query: `
    ...
    const { data: views, error: viewError } = await supabase.rpc('sql', {
      query: `
    ...
    const { data: testView, error: testError } = await supabase.rpc('sql', {
      query: `
