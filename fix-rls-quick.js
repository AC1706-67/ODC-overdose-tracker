...
async function fixRecursion() {
  try {
    ...
    await supabase.rpc('exec_sql', {
      sql_query:
        'CREATE POLICY "simple_user_access" ON user_organizations FOR ALL TO authenticated USING (auth.uid() = user_id);',
    });
    ...
    await supabase.rpc('exec_sql', {
      sql_query:
        'CREATE POLICY "public_org_access" ON organizations FOR SELECT TO authenticated USING (true);',
    });
    await supabase.rpc('exec_sql', {
