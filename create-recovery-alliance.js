...
async function createRecoveryAlliance() {
  try {
    ...
    if (recoveryAlliance.error) {
      ...
      console.log(
        '✅ Recovery Alliance created with ID:',
        recoveryAlliance.data[0].id,
      );
    }
    ...
    const testSubmission = await supabase.from('outreach_logs').insert({
      ...
      females_reached: 0,
      notes:
        "We're in the field we made two trips on this outreach and we reached three people",
    ...
    console.log('\n✅ Setup complete!');
    console.log(
      '💡 The issue is likely that activeOrgId has an invalid value.',
    );
